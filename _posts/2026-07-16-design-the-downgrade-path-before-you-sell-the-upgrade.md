...
That is why I keep coming back to [User control and freedom](https://www.nngroup.com/articles/user-control-and-freedom/). The old heuristic still holds. People need clear exits, ways to reverse choices, and confidence that the interface will not trap them inside an accidental commitment.
...
I like the way [Stripe documents subscription cancellation](https://docs.stripe.com/billing/subscriptions/cancel) because it treats the decision as a sequence of states rather than a single dramatic event.
